...
import { normalizePublicHttpsUrl, validatePublicHttpsUrl } from '../src/lib/publicUrl.js';
...
describe('validatePublicHttpsUrl', () => {
  ...
  it('accepts valid HTTPS origins', () => {
    ...
    expect(validatePublicHttpsUrl('https://composition-teachers-breeding-suddenly.trycloudflare.com')).toBeNull();
  });
  ...
  it('rejects /alexa suffix with a specific message', () => {
    ...
    expect(validatePublicHttpsUrl('https://example.com/alexa/foo')).toBe(
      'Use the base URL only — do not include /alexa',
    );
  });
